...
*/
var Message = function (addresses) {
    this.data = {};
    this.errors = [];
    ...
    if (this._checkAddresses(addresses)) {
        this.config.recipientAddressList = addresses;
    }
};
...
Message.prototype.setSenderAddressType = function (type) {
    if (this.availableSenderAddressTypes.indexOf(type) !== -1) {
...
Message.prototype.setSendAsFlashSms = function (sendAsFlashSms) {
    if (sendAsFlashSms === undefined || typeof sendAsFlashSms === 'boolean') {
        this.data.setSendAsFlashSms = sendAsFlashSms;
...
Message.prototype._checkAddresses = function (addresses) {
    ...
    if (!addresses || !addresses.length || !util.isArray(addresses)) {
        valid = false;
        this.error = {
            'cause': 'missing_addresses',
            'message': 'no addresses found'
        };
        this.error = {
            'cause': 'missingAddresses',
            'message': 'there has to be at least one recipient telefonnumber'
        };
    } else {
        var i = 0;
        for (i; i < addresses.length; i = i + 1) {
            if (!/^\d{1,15}$/.test(addresses[i])) {
                ...
                this.error = {
                    'cause': 'invalidNumber',
                    'message': 'telefon numbers should contain max. 15 digits'
                };
                break;
            }
        }
